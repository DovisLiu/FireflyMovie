...
public class SubtitleSetting {
    ...
    // media3 fork 无 DEFAULT_EDGE_WIDTH / DEFAULT_SHADOW_OFFSET 常量，用 Firefly 语义的默认值
    private static final float DEFAULT_EDGE_WIDTH = 0.1f;
    ...
    public static CaptionStyleCompat getStyle(Context context) {
        ...
        return new CaptionStyleCompat(getTextColor(), getBackgroundColor(), getEdgeType(), getEdgeColor(), Color.TRANSPARENT, null);
    }
    ...
    public static void applyStyle(Context context, @Nullable SubtitleView subtitleView) {
        ...
        if (isScaleApplied()) subtitleView.setFractionalTextSize(getScale(context) * SubtitleView.DEFAULT_TEXT_SIZE_FRACTION);
